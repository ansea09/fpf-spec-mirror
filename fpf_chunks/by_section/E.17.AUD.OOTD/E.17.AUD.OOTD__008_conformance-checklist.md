...
6. **CC-OOTD-6 - Local vs publication-unit repair choice is honest.**
   Apply `E.17.AUD.LHR` (`Local Head Restoration`) first when local repair is enough; apply this pattern only when publication-unit reading instability remains after local repair.
7. **CC-OOTD-7 - Neighboring-pattern boundary is explicit.**
   If same-entity transform, explanation, bridge, comparative-reading, ontology, gate, approval, or execution claim becomes primary, its governing pattern is applied rather than pretending this pattern still governs the case.
