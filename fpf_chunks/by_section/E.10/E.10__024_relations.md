...
- **Coordinates with precision-restoration patterns:** `E.10.ARCH`, `E.10.ROLE`, `A.6.P`, `A.6.P.WMR`, `A.6.RCD`, `C.2.P`, `A.19.SPR`, `E.10.MOVE`, and the direct domain restoration pattern selected by the current trigger; use `A.15.PROD` for local production-work, entity-inception, and production-completion claims when that exit is current.
