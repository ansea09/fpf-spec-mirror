...
| --- | --- | --- |
| No FPF-governed use after the wording-use check | Keep ordinary prose, quote, didactic phrase, or not-triggered text. | No precision-restoration pattern opens. |
...
| Method-like wording—for example, method, practice, technique, algorithm, program, solver, proof, recipe, workflow, process, procedure, access-path, query-plan, control-strategy, method algebra, method graph, selector calculus, or programming-paradigm wording whose method-side object or direct relation is hidden | Recover the object or direct relation meant by the wording before rewriting: `A.3.1 U.Method`, `MethodRelationStructure@BoundedContext` when method composition or method-family relation is current, `A.3.2 U.MethodDescription`, `A.6.0` formal-substrate declaration, `C.29` mathematical-lens use, `A.6.1` with `E.20` mechanism claim, `A.15.2 U.WorkPlan`, `A.15.1 U.Work`, or—only after `E.10.ROLE` recovers that work-facing claim—A.2 with C.3 for a local system-role kind, A.2.1 for an obtaining `U.SystemRoleAssignment` occurrence, or A.2.7 for a relation among system-role kinds. Other exits include `A.1.1` bounded context, `C.20` discipline position, `C.36.P` cultural-evolution wording, `G.5` method-family registry or selector outcome, `A.10` evidence relation, quote-only source wording, or another pattern for the recovered object. | Pre- and post-repair object and, when material, the pattern contribution—one `U.Method`, method-side relation, one-method `U.MethodDescription` episteme, formal-substrate declaration, C.29 representation and correspondence, mechanism, plan, dated Work, system-role kind and classification, assignment occurrence, relation among system-role kinds, transformation, result, or other direct claim—plus admissible use, blocked overread, and remaining reader use. Do not replace one umbrella with `method`, `practice`, `mechanism`, `algorithm`, `workflow`, or `method algebra` by taste. |
...
| `culture`, `cultural evolution`, `style`, `tradition`, `genre`, `scene`, `technique`, `practice`, `platform`, `regime`, `measurement regime`, `attractor`, `developmental machinery`, or close cultural-evolution labels | After the immediate disposition row selects the current object, recover the method family, work family, discipline, canon or memory episteme, recognition or selection regime, mediation system or architecture, measurement or visibility relation, publication label, variant set, dynamics or mathematical-lens claim, bounded context, development-loop relation, or cultural-evolution case. When source wording also says *role*, keep ordinary or quoted non-use or apply `E.10.ROLE` and recover the local system-role kind and classification, obtaining `U.SystemRoleAssignment` occurrence, relation among system-role kinds, or another direct relation actually claimed. Apply the direct pattern for the selected object; use `F.17`, `F.18`, and `F.9` only for term and Bridge work after recovery. | root culture or style kind by label, platform or regime as root ontology, loose attractor metaphor as dynamics claim, genre tree as proof of cultural identity, default role ontology, or replacing one broad source word with another broad FPF-looking word |
