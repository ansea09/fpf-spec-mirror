...
3. **Performed work after planning; filled positive connection.** **Readable result:** the named build `U.Work` occurrence populated the named artifact-store partition; that occurrence and the store change are connected by the declared BuildOps predicate, not by timing or the word *build*. `A.15.1` grounds `ReleaseBinary12_BuildWork_2026-07-21T0900_0912 : U.Work`. `A.3.4` separately grounds `ArtifactStorePopulationTransformation_12 : U.Transformation` as the 09:00-09:12 change of `ArtifactStorePartition_12` from no stored `ReleaseBinary_12` to stored `ReleaseBinary_12` under `BuildOpsStoreScheme-v12`. Predicate-definition episteme `BuildWorkPopulatedStore@BuildOps-v12(work, transformation)` holds only when that `U.Work` occurrence performs the exact `storeWrite` application declared by the predicate and changes the same partition. `BuildApplication_12` supplies that performed application and its `builtBinary -> ReleaseBinary_12` binding, so C.2.1 assertion `BuildWorkPopulatedStore-12` carries the positive work-to-change claim under `A.6.RCD` disposition 1. P2W keeps the predicate definition, assertion, Work occurrence, Transformation, and performed application distinct. If no current predicate or constructor rule can state the connection, return `missing-governor`; if the governor exists and the available base facts are sufficient to apply its positive test but that test fails, return `factually unsupported`; if a fact needed to decide the test is unavailable, return `missing-information`. A negative needs its own applicable non-obtaining criterion or complete closure basis and satisfying facts.

4. **Result interpretation without generic result.** The sentence *the work result proves the approach worked* does not yet name a result. Ask what can actually be asserted. `A.6.P.WMR` may return a direct subject claim, an `A.6.1` application binding, a local `A.15.PROD` or `A.6.RCD` claim, or a bounded non-assertability result. P2W carries only the returned item. `factually unsupported` and `missing-information` stop an unsupported or underinformed claim; `missing-governor` says that no current predicate definition or occurrence rule can state it for the named participants and use. None becomes a generic result or production value.

5. **Functional explanatory order.** A source diagram places formal declaration, principle framing, mechanism, normalization, method selection, planning, performed work, and result measurement in one readable order. The diagram helps recognize candidate continuations, but P2W carries only the separately returned signature declaration, principle frame, mechanism or application, normalized value, method-selection result, plan, dated Work claim, or measurement result; the display order supplies no sequence or authority.

6. **Interface split before P2W use.** A source says a port-throughput limit makes a solution feasible after integration. The practitioner opens separate `A.6.M` module-interface and `E.18` transformation-flow questions. Planning, work, evidence, gate, function, and architecture cues remain stopped until their relations are asserted. Conversational P2W use or the compact note carries only the result or honest stop returned for the current question that changes the present decision.
...
8. **Pump 14 pressure adjustment; positive continuation after an earlier stop.** **Readable result:** the current case record supports `W-P14-ADJUST-1010-1020 caused T-P14-PRESSURE-RISE` under `AdjustmentWorkCausesPressureRise`; P2W carries that returned result rather than inferring it from timing. **Exact basis:** `PumpAdjustmentWorkAssignment@PlantOps-v3` is a directly declared species of `U.SystemRoleAssignment`. Its participant order is `<HolderSystemSlot, AssignedSystemRoleKindSlot>` under local domain `PumpAdjustmentSystemRoleKindDomain@PlantOps-v3`, and its predicate selects the holder to supply the denoted adjustment contribution during the covered interval. `RA-P14-ADJUST : PumpAdjustmentWorkAssignment@PlantOps-v3` fixes `<PumpTeam-14, PumpAdjustmentSystemRole>`, obtains throughout 10:10–10:20, and has `PumpTeam-14` as its holder. `PumpTeam-14`, independently admitted as `U.System`, performs dated Work `W-P14-ADJUST-1010-1020`, which enacts `SetPointAdjustment@PlantOps-v3` from 10:10 to 10:20 within `PumpStation-14`; the F.6 occurrence `performedUnderAssignment(W-P14-ADJUST-1010-1020, RA-P14-ADJUST)` obtains and its performer projection equals that holder. A compact P2W note may omit only an assignment identifier unused by its receiving claim after this basis remains recoverable. Independently, `A.3.4` identifies `T-P14-PRESSURE-RISE` as the bounded `U.Transformation` of continuing `HydraulicLoop_P14`, whose discharge-pressure characteristic changes from `belowBand` to `inBand` over the same interval. Predicate-definition episteme `P14-REL-2026` declares `AdjustmentWorkCausesPressureRise` for those participants, and a separate case fact satisfies its actual-causation predicate. In the earlier case record, `P14-REL-2026` is absent; at that stage, keep the Work and transformation separate, return `missing-governor` for the claim that `W-P14-ADJUST-1010-1020` caused `T-P14-PRESSURE-RISE`, and name the missing predicate definition. A namespace, relation-family label, or pattern title neither supplies that definition nor acts as its maintainer. The separate claim that `PC-P14-PRESSURE` guided `WP-P14-2026-07-15` remains `missing-governor` under A.6.P.WMR; neither the problem claim nor shared timing causes the Work. Later measurement and decision uses remain separate; no production or transformation-composition question opens.
