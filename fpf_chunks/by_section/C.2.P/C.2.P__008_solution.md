...
| `admissibleUse` | The exact admissible use, non-admissible stronger or adjacent use, and L-, A-, D-, and E-claim split when the sentence makes a boundary-use claim. | `A.6.B`, `A.6`, and the governing pattern for the use. |
...
| generic publication face | Reader-facing publication projection or face. It is not `U.View` by default; it becomes a view only when the governing FPF pattern makes that relation live. | `U.View` by default, carrier, UI face, front-end display, MVPK face under E.17 constraints, or claim-bearing episteme. |
