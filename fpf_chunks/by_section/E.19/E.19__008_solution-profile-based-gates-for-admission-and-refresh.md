...
**Misuse guard.** A pattern-quality caution, return-for-repair result, or selected pattern-quality boundary result cannot be reused as project refusal or project approval unless a project-side receiving relation is live and states admissible and non-admissible use for that relation.
...
6. **Reader-role fit**
   The live pattern body stays addressed to the intended FPF user rather than to FPF developers or package architects. FPF-governed sections explain admissible use, costs, boundaries, exact FPF governing patterns, exact project-side FPF kinds and references, and related exact relations in user terms. Architecture-placement, freeze/merge state, and broader package-development rationale stay in separate companions or clearly marked informative placement notes when needed.
...
* **Precision-restoration distribution must be preserved.** When an `E.10` scan selects a non-local precision-restoration path, the run checks that `E.10` remains the trigger and applicability pattern, `E.10.ARCH` carries the shared recovery architecture, the relevant realization pattern (`A.6.P`, `C.2.P`, `C.30.P`, `C.16.P`, `C.16.Q`, `A.19.SPR`, or another exact restoration pattern) performs the ontological unpacking, and receiving patterns keep thin pointers rather than local trigger registries or duplicate recovery algorithms.
* **EntityOfConcern and semio-bias questions travel with the same triage.** When the reviewed change touches EntityOfConcern, same-referent, slot/reference, alignment-path, role-boundary, consumer-disposition wording, or description/publication-use guards, the run asks the following questions before acceptance: what is the pattern's own `EntityOfConcern` and first useful move; is any `EntityOfConcernSlot` or `EntityOfConcernRef` used only where a claim-bearing episteme or episteme-lane view is live; are legacy selected-entity spellings confined to source-side wording rather than a second ontology; does the text preserve same-EntityOfConcern semantics or explicitly enter `A.6.4` retargeting; which exact alignment path carries non-identical transfer (`EntityOfConcernAlignmentCase`, `A.6.4`, `F.9`, `F.17`, `F.18`, `A.6.P`, another exact relation pattern, or blocked transfer); do role, method, work, evidence, assurance, gate, and decision claims remain with their exact governing patterns instead of being inferred from source, publication, record, or review wording; do generic guards that a description, view, publication, record, card, diagram, source, or file is not a permission, promise, prescription, evidence item, assurance verdict, decision, gate passage, release, work occurrence, or authority source stay in a named description and publication-use boundary section rather than becoming the subject Solution; and has every current-host consumer of the selected-family repair received a semantic, mechanical, compatibility, or not-triggered disposition.

* **Design-time and run-time both count.** The same precision discipline applies to live FPF pattern prose and to any reviewed publication text, worked slice, or performed-work exemplar when that text is being assessed for admissibility, guidance, reuse, gating, release, policy, assurance, or action-selection use.
...
* each SoTA row changes at least one FPF-governed outcome for the pattern: what the user may do, what the user must not over-read, which exact FPF pattern application must be named, or which claim cannot be raised to release, policy, assurance, gate, action-selection, or adjudication use,
...
* FPF-governed trigger wording must be classified before acceptance, including `lawful`, `lawfully`, or `lawfulness`; `burden`; `affordance`; `axis` or `axes`; `tier`, `rung`, `level`, `layer`, or `ladder`; `contract`; `lifecycle`; `workflow`; `procedure` and `call` wording; `home` and `owner`; `surface`; `state`, `status`, `posture`, `readiness`, `stance`, or `currentness`; `route`, `routing`, `reroute`, `exit`, `handoff`, `path`, `pipeline`, or `flow`; and local equivalents. The run names the exact admissibility, deontic, evidence, review-check, reader-task, action-invitation, characteristic or scale, maturity-rung, state-family bearer and value frame under `A.19.SPR`, EntityOfConcern and Description-episteme boundary, specification-use gate, control-layer relation, method, work-plan, work occurrence, authority reference, declarative FPF pattern application with exact governing ontology or conformance claim or section, publication face, publication form, publication unit, or publication carrier, or selected companion role or base declaration that is live,
...
* **Trigger class 3 — multi-role or high-risk public entry change**
  one change affecting several live projection or companion roles together, one
...
5. **PCP-ENTRY-E5 — companion-or-projection role parity check**
   Do the live companion or projection roles, plus any explicit absence note, preserve
...
| 3 | “this is the same EntityOfConcern re-expressed for another audience” | same-EntityOfConcern rewrite neighborhood | `A.6.3.CR`, `A.6.3.RT`, `E.17.EFP`, `E.17.ID.CR` | minting one second `U.Episteme` for the same claim or one second competing explanatory lane instead of one same-EntityOfConcern rewrite | `wrong-pattern-rejected` or same-EntityOfConcern rewrite opened | one worked-entry walk-through, one pattern `Problem frame`, governing-entry pointer | one parallel explanatory blurb treated as one second governing pattern |
...
| 6 | “the LLM retrieved a helpful-looking paragraph but not the pattern” | one retrieval-facing entry-neighborhood case | one applicable governing pattern body plus one applicable projection role | one stale thin echo or one projection-only companion role answered as if it were the governing entry | `governing-entry-opened` or `worked-walk-through-needed` | one governing-entry reference, one projection-only status marker, one retrieval-facing pointer to the applicable governing pattern body | one thin echo chunk without governing-entry reference or projection-only cue |
...
and, when thin echoes are live, thin-echo governing-entry reference presence.
