...
| --- | --- | --- |
| **CC-A6F-1 Exact FPF-kind or relation recovery.** | Every function-like phrase that carries a live FPF claim names the recovered FPF kind, relation, claim record, view, or receiving-pattern application and, when the claim points to a specific source, the recovered FPF reference. | Add `FunctionUseRepair` or demote the phrase to Plain prose. |
...
| **CC-A6F-8 Quality and functionality boundary.** | Quality, fitness, characteristic, score, or "functionality" wording recovers bearer and exact governing pattern. | Assign the claim to `C.25`, `C.16`, C.16.Q, `A.17`, `A.18`, or an admitted characteristic or measurement receiving pattern as live. |
