...
Use `ArchitectureDescription@Context` when the live EntityOfConcern is the description episteme or specification-use record over one `ArchitectureOf@Context`. The described holon is recovered through `ArchitectureOf@Context.describedHolonRef`; the `DescriptionContext.EntityOfConcernRef` for the architecture description points to the architecture claim record.
...
- `correspondenceRefs` or a source-return condition is present when cross-view or source reuse is live;
...
A full architecture description is traceable only when the reader can recover the chain that makes a view useful without turning the view into the architecture. The chain is a trace obligation, not an ordered work sequence:

```text
workingConcernRef
  or A15RoleEnactorFamilyRef when A.15 role-enactor semantics are live
-> viewpointRef
...
-> sourceEpistemeRef or sourceViewRef when source use is live
-> PublicationUnitRef, publication face, or publication form only when published
-> correspondenceRef or sourceReturnCondition when cross-view or source reuse is live
-> admissibleArchitectureMove or exact neighboring-pattern application
```
...
Use `C.30.ASV` when the live question is whether the view has the right structure kind, viewpoint, hidden or lost structure note, correspondence, or source return. Use `A.22` when the live question is structure as such. Use `C.30` when the live question is the grounded architecture claim. Use `C.30.AD` only for the description's membership, composition, correspondence, source-return, freshness, specification-use, publication-use, or remaining-move boundary.
...
| Control or LCA view | `C.30.LCA` when a control structure view is live. |
| Module or interface view | `A.6.M`, signature or interface patterns, and `C.30.ASV` when module-interface structure is live. |
...
If the specification use becomes evidence, assurance, gate, work, decision, causal-use, or release authority, apply the exact neighboring pattern to that authority claim. The architecture description remains the description boundary, not the authority.
...
| Live question after the architecture-description boundary is clear | FPF application |
| --- | --- |
...
| **CC-C30AD-2a Traceable multi-view chain.** | The description use recovers the chain from working concern or live A.15 role-enactor family through viewpoint, selected structure or structure kind, architecture claim, ASV view, architecture description, source or publication use when live, correspondence or source return when live, and remaining admissible architecture move. | Add the missing reference, reduce the admissible use, or return to the governing pattern that can recover the missing relation. |
...
| **CC-C30AD-5 Publication boundary.** | Publication face, carrier, diagram, dashboard, card, file, or rendering is not treated as architecture, decision, evidence, assurance, gate, work, or release authority. | Assign publication or source use to `C.2.P` or `E.17` and the non-architecture claim to the exact neighboring pattern. |
| **CC-C30AD-6 Specification-use boundary.** | Specification use is declared as use over a Description episteme or publication, with exact neighboring applications when it carries authority. | Add `ArchitectureDescriptionSpecificationUse@Project` or demote to ordinary description. |
